...
<html>
    ...
    <body onload="history.go(+1)">
        ...
        <div class="row margin">
            <div class="col-xs-6 col-md-8 col-md-offset-2 col-lg-8 col-md-offset-2">
                <h2>Selecione o funcionário na lista abaixo</h2>
            </div>
        </div>
              
        <div class="row margin">
            <div class="col-xs-6 col-md-8 col-md-offset-2 col-lg-8 col-md-offset-2">
                <form name="rm" action="alteracao.php" method="post">
      
                <select name="rm" class="listaexcluir">
                    <option value="0" selected>Selecione</option>
                    <?php
                        $sql = "SELECT RM, NOME, SOBRENOME FROM rm ORDER BY NOME";
                        $result = mysqli_query($conn, $sql);
                        while($row = mysqli_fetch_assoc($result)){
                    ?>
                    <option value="<?php echo "".$row['RM']; ?>"><?php echo "".$row['NOME']." ".$row['SOBRENOME'];?></option>
                        <?php } ?>
                </select>
